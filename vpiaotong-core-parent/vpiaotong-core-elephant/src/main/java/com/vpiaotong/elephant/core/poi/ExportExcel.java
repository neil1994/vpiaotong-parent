package com.vpiaotong.elephant.core.poi;
...
//        HSSFSheet sheet = workbook.createSheet(title);
        HSSFSheet sheet = workbook.createSheet(WorkbookUtil.createSafeSheetName(title));
        // 设置表格默认列宽度为15个字节
...
//        comment.setAuthor("leno");
        ...
        for (int i = 0; i < headers.length; i++) {
           ...
           // sheet.setColumnWidth(i,cell.getRichStringCellValue().getString().getBytes().length*2*256);
...
//                    sheet.autoSizeColumn((short)4);//调整第五列的宽度
                    
                } catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
                	LOGGER.error("导出excel 数据组装异常:{}",e);
				} finally {
                    // 清理资源
                }
            }
        ...
        try {
            ...
            LOGGER.error("导出excel 写入文件异常:{}",e);
        }finally {
			if (workbook!=null) {
				try {
					workbook.close();
				} catch (IOException e) {
					LOGGER.error("导出excel 关闭流异常:{}",e);
				}
			}
		}
